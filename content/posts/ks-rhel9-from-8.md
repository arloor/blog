...
自动安装是通过 `pxeboot` + ` kickstart`实现的，自动安装因为要通过内存承载liveos，所以内存要大一点。通过腾讯云CVM测试，英特尔CPU需要2G内存，AMD CPU需要4G内存，才能自动安装。对于不同品牌CPU的最小内存要求不同这点，我也很困惑。不过，为了减少折腾，建议是直接4G内存起步，反正按量计费下2小时自动销毁也就两块钱。
