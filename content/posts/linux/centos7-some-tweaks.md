...
echo "--------------------------------------------------------------------------"
echo 流量累计使用情况：
cat /proc/net/dev|tail -n +3|awk '{eth=$1;xin=$2 / 1073741824;xout=$10 / 1073741824;printf("%6s \033[32m%7.2fGB\033[0m出 \033[32m%7.2fGB\033[0m入\n",eth,xout,xin)}'
echo "--------------------------------------------------------------------------"
EOF
chmod +x /usr/local/bin/netsum
/usr/local/bin/netsum
echo '0 4 * * * root /usr/local/bin/netsum >> /root/net.log' >> /etc/crontab 
```
...
wget https://cdn.arloor.com/jdk/jdk8.rpm -O jdk8.rpm
rpm -ivh jdk8.rpm
rm -f jdk8.rpm
```
# 设置时区
